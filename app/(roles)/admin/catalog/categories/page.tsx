...
import { getCategories, createCategory, updateCategory, archiveCategory, type CategoryRow } from "@/app/actions/catalog";
...
export default function CategoriesPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    let result;
    if (editingItem) {
      result = await updateCategory(editingItem.id, form);
    } else {
  ...
  async function handleArchive(id: number) {
    if (!confirm("Archive this category? It will move to the System Archives.")) return;
    await archiveCategory(id);
    loadData();
  }

  function openEditDialog(item: CategoryRow) {
    setEditingItem(item);
    setDialogOpen(true);
  }
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">Manage product categories for the catalog.</p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={handleOpenChange}>
          <DialogTrigger asChild>
            <Button className="bg-[#005914] hover:bg-[#00420f]"><PlusCircle className="w-4 h-4 mr-2" />Add Category</Button>
          </DialogTrigger>
          <DialogContent className="sm:max-w-sm">
            <DialogHeader><DialogTitle>{editingItem ? "Edit Category" : "Add Category"}</DialogTitle></DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="name">Name</Label>
                <Input id="name" name="name" required placeholder="e.g. Canned Tuna" defaultValue={editingItem?.name || ""} />
              </div>
              <div className="space-y-2">
      ...
      <Card className="shadow-sm border-0 rounded-xl">
        <CardHeader className="py-4 border-b border-gray-100"><CardTitle className="text-lg font-semibold text-gray-800">All Categories</CardTitle></CardHeader>
