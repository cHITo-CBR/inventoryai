...
export async function getQuotas(filters?: {
  ...
  try {
    ...
    return (data || []).map((q: any) => ({
      ...
      achieved_amount: Number(q.achieved_amount),
      amount_percentage: q.amount_percentage ? Number(q.amount_percentage) : null,
      units_percentage: q.units_percentage ? Number(q.units_percentage) : null,
      orders_percentage: q.orders_percentage ? Number(q.orders_percentage) : null,
    }));
  } catch (error) {
...
export async function getCurrentMonthQuotaSummary(): Promise<{
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("salesman_quotas")
      .select("target_amount, achieved_amount, status")
    ...
    const completed_quotas = records.filter((r: any) => r.status === "completed").length;
    const total_target = records.reduce((sum: number, r: any) => sum + (Number(r.target_amount) || 0), 0);
    const total_achieved = records.reduce((sum: number, r: any) => sum + (Number(r.achieved_amount) || 0), 0);
